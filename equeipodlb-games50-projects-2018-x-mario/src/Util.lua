...
function GenerateQuadsKeys(atlas)
    local keys = {}
    for counter = 1, 4 do --newQuad(x,y,width,height)
        keys[counter] = love.graphics.newQuad(0+16*(counter-1),0,16,16,atlas:getDimensions())
    end
    return keys
end
function GenerateQuadsLocks(atlas)
    local locks = {}
    for counter = 1,4 do
        locks[counter] = love.graphics.newQuad(0+16*(counter-1),16,16,16,atlas:getDimensions())
    end
    return locks
end

function GenerateQuadsGoalPosts(atlas)
    local goalposts = {}
    for counter = 1,6 do
        goalposts[counter] = love.graphics.newQuad(0+16*(counter-1),0,16,48,atlas:getDimensions())
    end
    return goalposts
...
function GenerateQuadsFlags(atlas)
    local flags = {}
    for counter = 1,4 do
        flags[counter] = love.graphics.newQuad(96,0 + 16*(counter - 1),48,16,atlas:getDimensions())
    end
    return flags
end
