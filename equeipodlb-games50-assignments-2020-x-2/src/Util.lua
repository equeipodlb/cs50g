...
function GenerateQuadsBricks(atlas)
    local allbricks = GenerateQuads(atlas,32,16)
    local bricksWeWant = table.slice(allbricks,1,21)
    table.insert(bricksWeWant, allbricks[24]) -- We take all bricks from 1 to 21 and add 24 = the locked one
    return bricksWeWant
end
...
function GenerateQuadsPowerUps(atlas)
    ...
    quads[2] = love.graphics.newQuad(x + 16, y, 16, 16, atlas:getDimensions())
    return quads
end