...
public class LevelGenerator : MonoBehaviour {
	...
	public Pair<int,int>[] holeCoordinates;
	// Use this for initialization
	void Start () {

		// initialize map 2D array
		mapData = GenerateMazeData();
		int numbOfHoles = mazeSize / 10;
		holeCoordinates = new Pair<int,int>[numbOfHoles];

		for (int i = 0; i < numbOfHoles; ++i) {
			int x = Random.Range(1,mazeSize - 2);
			int z = Random.Range(1,mazeSize - 2);
			while (mapData[z,x]){
				x = Random.Range(1,mazeSize - 2);
				z = Random.Range(1,mazeSize - 2);
			}
			holeCoordinates[i] = new Pair<int,int>(x,z);
		}
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				bool found = false;
                    for (int i = 0; i < numbOfHoles; i++) {
                        if (holeCoordinates[i].First == x && holeCoordinates[i].Second == z) {
                            found = true;
                            break;
                        }
                    }
