...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.aliens[1].collided and not self.split then
            ...
            table.insert(self.aliens, Alien(self.world, 'round', posX, posY, 'Player'))
            self.aliens[2].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY + 5) * 10)
            self.aliens[3].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY - 5) * 10)
